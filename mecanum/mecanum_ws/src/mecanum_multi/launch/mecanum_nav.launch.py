...
def generate_launch_description():
    nav2_bringup_dir = get_package_share_directory('mecanum_multi')

    use_sim_time = LaunchConfiguration('use_sim_time', default='false')
    namespace = LaunchConfiguration('namespace', default='mecanum')
    # ADR-0027/0033: 权威地图统一为 real_competition_map (robot_nav 内已安装, 与 PC 同一 .pgm)
    map_yaml_path = LaunchConfiguration(
        'maps', default=os.path.join(
            get_package_share_directory('robot_nav'), 'maps', 'real_competition_map.yaml'))
    nav2_param_path = LaunchConfiguration('params_file', default=os.path.join(
        nav2_bringup_dir, 'param', 'mecanum_nav.yaml'))
    ...
    health_monitor = Node(
        ...
        parameters=[{
            ...
            'map_version': 'real_competition_map_v1',
            'map_frame': 'map',
            'odom_frame': 'mecanum/odom',
            'base_frame': 'mecanum/base_footprint',
        }],
    )
