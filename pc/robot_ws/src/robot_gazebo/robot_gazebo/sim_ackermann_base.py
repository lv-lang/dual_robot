...
class SimAckermannBase(Node):
    """Simple planar Gazebo adapter for robot2 Nav2 Twist commands."""

    def __init__(self) -> None:
        super().__init__('sim_ackermann_base')
        ...
        self.declare_parameter('odom_frame', 'robot2/odom')
        self.declare_parameter('base_frame', 'robot2/base_footprint')
        ...
        self.declare_parameter('initial_x', -8.5)
        ...
        self.get_logger().info(
            f'sim_ackermann_base active: {self.cmd_vel_topic} -> {self.odom_topic} '
            f'at ({self.x:.2f}, {self.y:.2f}, {self.yaw:.2f})')

    def _on_cmd_vel(self, msg: Twist) -> None:
        self.last_cmd = _ackermann_twist(msg)
    ...
    def _integrate(self, cmd: Twist, dt: float) -> None:
        if dt <= 0.0:
            return
        self.x += math.cos(self.yaw) * cmd.linear.x * dt
        self.y += math.sin(self.yaw) * cmd.linear.x * dt
...
def _ackermann_twist(msg: Twist) -> Twist:
    cmd = Twist()
