...
class ackermann_driver(Node):
	...
	def stop_car(self):
		...
		try:
			...
			self.get_logger().info("Sent zero motion command on shutdown.")
		except Exception as exc:
			self.get_logger().error("Failed to send zero motion command on shutdown: %s" % exc)
			
def main():
	rclpy.init() 
	driver = ackermann_driver('driver_node')
